...
export default {
  ...
  beforeMount () {
    if (sessionStorage.getItem('editPreorder') === 'true') {
      ...
      this.hour = template.start_at.split(' ').pop().split(':').shift()
      this.minute = template.start_at.split(' ').pop().split(':')[1]
    }
  },
  computed: {
    ...mapGetters({
    ...
    days () {
      var week = [{ value: moment().format('DD.MM.YYYY'), name: this.$i18n.t('today', this.user.lang) }]
      var tomorrow = new Date()
      for (let i = 0; i <= 365; i++) {
        tomorrow.setDate(tomorrow.getDate() + 1)
        let name = new Date(tomorrow).toLocaleString(this.user.lang, { weekday: 'short', day: 'numeric', month: 'long' })
        let value = new Date(tomorrow).toLocaleString('ru', { day: '2-digit', month: '2-digit', year: 'numeric' })
        week.push({ value: value, name: name })
      }
      return week
    },
    dateFormat () {
      return this.day + 'T' + this.hour + ':' + this.minute + ':00'
    }
  },
  methods: {
    closeTimeSelect () {
      this.$emit('closeTime', { date: this.dateFormat })
    },
    moment () {
      return moment()
    },
